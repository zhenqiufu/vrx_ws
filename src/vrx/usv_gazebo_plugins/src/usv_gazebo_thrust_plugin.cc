...
void Thruster::OnThrustAngle(const std_msgs::Float32::ConstPtr &_msg) {
  ...
  std::lock_guard<std::mutex> lock(this->plugin->mutex);
  this->desiredAngle =
      boost::algorithm::clamp(_msg->data, -this->maxAngle, this->maxAngle);
}
...
void UsvThrust::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("thruster")) {
    ...
    while (thrusterSDF) {
      ...
      thruster.maxAngle =
          this->SdfParamDouble(thrusterSDF, "maxAngle", M_PI / 2);
  ...
  for (size_t i = 0; i < this->thrusters.size(); ++i) {
    ...
    this->thrusters[i].cmdSub =
        this->rosnode->subscribe(this->thrusters[i].cmdTopic, 1,
                                 &Thruster::OnThrustCmd, &this->thrusters[i]);
...
double UsvThrust::Glf(const double _x, const float _A, const float _K,
                      const float _B, const float _v, const float _C,
                      const float _M) const {
  return _A + (_K - _A) / (pow(_C + exp(-_B * (_x - _M)), 1.0 / _v));
}

//////////////////////////////////////////////////
double UsvThrust::GlfThrustCmd(const double _cmd, const double _maxPos,
                               const double _maxNeg) const {
  double val = 0.0;
  if (_cmd > 0.01) {
  ...
  for (size_t i = 0; i < this->thrusters.size(); ++i) {
    {
      ...
      switch (this->thrusters[i].mappingType) {
        case 0:
          tforcev.X() = this->ScaleThrustCmd(
              this->thrusters[i].currCmd / this->thrusters[i].maxCmd,
              this->thrusters[i].maxCmd, this->thrusters[i].maxForceFwd,
              this->thrusters[i].maxForceRev);
          break;
        case 1:
          tforcev.X() = this->GlfThrustCmd(
              this->thrusters[i].currCmd / this->thrusters[i].maxCmd,
              this->thrusters[i].maxForceFwd, this->thrusters[i].maxForceRev);
          break;
        default:
          ROS_FATAL_STREAM(
              "Cannot use mappingType=" << this->thrusters[i].mappingType);
          break;
      }
...
void UsvThrust::RotateEngine(size_t _i, common::Time _stepTime) {
  ...
  double effort =
      this->thrusters[_i].engineJointPID.Update(angleError, _stepTime);
...
void UsvThrust::SpinPropeller(size_t _i) {
  ...
  if (std::abs(this->thrusters[_i].currCmd / this->thrusters[_i].maxCmd) >
      kMinInput)
    effort =
        (this->thrusters[_i].currCmd / this->thrusters[_i].maxCmd) * kMaxEffort;
